"""End-to-end sanity and regression tests for the simulation pipeline and the
proposed event-anchored + residual-CUSUM detector.

These are not exhaustive unit tests; they pin the load-bearing behaviours so that
accidental regressions (in the substrate, carbon layer, anomaly model, or either
detector) are caught, and they encode the paper's headline mechanism as a
regression test.
"""
import numpy as np
...
def test_substrate_structure():
    ...
    prod_mean = df.loc[df.state == "PRODUCTION", "total_kw"].mean()
    assert 1.0 < prod_mean < 20.0          # sane kW range for a small mill
...
def test_anomaly_adds_power_not_pieces():
    ...
    assert np.allclose(sub.pieces_cum.values, inj.pieces_cum.values)   # output unchanged
...
def test_deployed_quiet_when_clean():
    ...
    assert (obs["alert_level"].values >= 1).mean() < 0.2   # few/no false alarms


def test_anchored_recovers_slow_ramp_headline():
    """Regression test for the paper's central claim: on a slow ramp (onset ~=
    baseline window) the proposed fixed-reference + CUSUM detector fires."""
...
def test_held_baseline_constant_within_segment():
    ...
    finite = np.unique(np.round(B[np.isfinite(B)], 10))
    assert finite.size == 1     # one shift-start anchor -> a single held value
